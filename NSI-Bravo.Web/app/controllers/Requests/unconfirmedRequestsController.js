...
app.controller('unconfirmedRequestsController', ['$scope', '$location', '$timeout', '$routeParams', '$log', 'myCVService', 'unconfirmedRequestsService', '$route', function ($scope, $location, $timeout, $routeParams, $log, criteriaService, unconfirmedRequestsService, $route) {
    // MY CV TABLE
    ...
    function GetMyCVs() {
        clearTable();
        unconfirmedRequestsService.GetCVTable().then(function (response) {
            var CVTable = response.data;
            for (var i = 0; i < CVTable.length; i++) {
                criteriaService.GetMyCVs(CVTable[i].iD_CV).then(function (response) {
                    data = response.data;
